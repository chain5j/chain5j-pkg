...
func NewSyncBloom(memory uint64, database kvstore.Iteratee) *SyncBloom {
...
func (b *SyncBloom) init(database kvstore.Iteratee) {
	...
	for it.Next() && atomic.LoadUint32(&b.closed) == 0 {
		...
		if key := it.Key(); len(key) == types.HashLength {
