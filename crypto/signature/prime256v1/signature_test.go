...
func testSignCompact(priv *PrivateKey, tag string, data []byte, isCompressed bool) {
	...
	// 校验签名内容
	key := priv.PublicKey
	publicKey := NewPublicKey(curve, key.X, key.Y)
	verify := VerifySignature(publicKey.ToECDSA(), hashed, sig[:64])
...
func TestSignCompact(t *testing.T) {
	priv1, _ := GeneratePrivateKey(curve)
	t.Log("priv1.Serialize()", hexutil.Encode(priv1.Serialize()))

	key := "cca9fbcc1b41e5a95d369eaa6ddcff73b61a4efaa279cfc6567e8daa39cbaf50"
	priv, _ := PrivKeyFromBytes(curve, decodeHex(key))
...
func TestRFC6979(t *testing.T) {
	...
	for i, test := range tests {
		privKey, _ := PrivKeyFromBytes(curve, decodeHex(test.key))
		hash := sha256.Sum256([]byte(test.msg))

		// Ensure deterministically generated nonce is the expected value.
		gotNonce := NonceRFC6979(curve, privKey.D, hash[:], nil, nil).Bytes()
