...
func (db *Database) Get(key []byte) ([]byte, error) {
	...
	if entry, ok := db.db[string(key)]; ok {
		return hexutil.CopyBytes(entry), nil
	}
...
func (db *Database) Put(key []byte, value []byte) error {
	...
	db.db[string(key)] = hexutil.CopyBytes(value)
	return nil
}
...
func (db *Database) NewIterator() kvstore.Iterator {
...
// not exist).
func (db *Database) NewIteratorWithStart(start []byte) kvstore.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (db *Database) NewIteratorWithPrefix(prefix []byte) kvstore.Iterator {
	db.lock.RLock()
	defer db.lock.RUnlock()
...
func (b *batch) Put(key, value []byte) error {
	b.writes = append(b.writes, keyvalue{hexutil.CopyBytes(key), hexutil.CopyBytes(value), false})
	b.size += len(value)
	return nil
}
...
func (b *batch) Delete(key []byte) error {
	b.writes = append(b.writes, keyvalue{hexutil.CopyBytes(key), nil, true})
	b.size += 1
	return nil
}
